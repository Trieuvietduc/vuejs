<template>
        <VeeForm as="div" v-slot="{ handleSubmit }" @invalid-submit="onInvalidSubmit">
            <form @submit="handleSubmit($event, onSubmit)" method="post" ref="formData" action="/company"> 
                <Field type="hidden" :value="csrfToken" name="_token"/>
                <div>
                    <CFormLabel class="col-sm-3 lbl-input" require>Code</CFormLabel>
                    <Field name="code" v-model="model.code" rules="required|min:8|regex:^([a-zA-Z0-9]+)$"
                        class="form-control" />
                ...
                <div>
                    ...
                    <Field name="telephone" type="text" autocomplete="off" v-model="model.telephone"
                        rules="required|digits:10" class="form-control" ref="telephone" />
                    <ErrorMessage class="error" name="telephone" />
                </div>
                <div>
                    <CFormLabel class="col-sm-3 lbl-input" require>Địa chỉ</CFormLabel>
                    <Field name="address" type="text" autocomplete="off" v-model="model.address"
                        rules="required|max:255" class="form-control" />
